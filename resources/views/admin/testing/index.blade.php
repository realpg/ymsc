...
@section('content')
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 商品管理 <span class="c-gray en">&gt;</span>商品列表 <a class="btn btn-success radius btn-refresh r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" onclick="location.replace('{{URL::asset('/admin/testing/index')}}');" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="text-c">
        <form action="{{URL::asset('/admin/testing/index')}}" method="post" class="form-horizontal">
            {{csrf_field()}}
            ...
            <input id="search" name="search" type="text" class="input-text" style="width:250px" placeholder="商品名称/商品货号" value="{{$search}}">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            <a class="btn btn-primary radius" onclick="testing_edit('添加商品','{{URL::asset('/admin/testing/edit')}}')" href="javascript:;">
                <i class="Hui-iconfont">&#xe600;</i> 添加商品
            </a>
        </span>
        <span class="l ml-10">
            <a href="javascript:;" onclick="testing_delMore()" class="btn btn-danger radius"><i class="Hui-iconfont"></i> 批量删除</a>
        </span>
    </div>
    <div class="mt-10">
        <table class="table table-border table-bordered table-bg table-hover table-sort" id="table-sort">
            ...
            @foreach($datas as $data)
                <tr class="text-c">
                    ...
                    <td class="td-manage">
                        <a title="编辑" href="javascript:;" onclick="testing_edit('编辑','{{URL::asset('/admin/testing/edit')}}?id={{$data['id']}}&menu_id={{$data['menu_id']}}',{{$data['id']}})" class="ml-5" style="text-decoration:none">
                            <i class="Hui-iconfont">&#xe6df;</i>
                        </a>
                        <a title="删除" href="javascript:;" onclick="testing_del(this,'{{$data['id']}}')" class="ml-5" style="text-decoration:none">
                            <i class="Hui-iconfont">&#xe6e2;</i>
                        </a>
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    /*查看商品详情*/
    function testing_edit(title, url, id) {
        // console.log("testing_edit url:" + url);
    ...
    function testing_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            ...
            delTesting('{{URL::asset('')}}', param, function (ret) {
                if (ret.result == true) {
    ...
    function testing_delMore(){
        ...
        if(id_array){
            delMoreTesting('{{URL::asset('')}}', param, function (ret) {
                if (ret.result == true) {
                    // $(obj).parents("tr").remove();
                    layer.msg(ret.msg, {icon: 1, time: 1000});
                    $('.btn-refresh').click();
