...
//                        console.log("BeforeUpload up:" + up + " file:" + JSON.stringify(file));
        ...
        function delExplain(index,id){
            layer.confirm('确认要删除这条数据吗？',function(index){
                ...
                delTestingExplain('{{URL::asset('')}}', param, function (ret) {
                    if (ret.result == true) {
                        layer.msg(ret.msg, {icon: 1, time: 1000});
                        console.log('delTestingExplain index is : ' + JSON.stringify((jsonObj_explains[index])))
                        for(var i=0;i<jsonObj_explains.length;i++){
        ...
        function submitExplainText() {
            ...
            if(add_text==''){
                ...
                addTestingExplainList(detail,function(ret){
                    console.log('addTestingDetailList text ret is : '+JSON.stringify(ret))
                    if (ret.result == true) {
                        //重新展示
        ...
        function submitExplainImage() {
            ...
            if(add_image==''){
                ...
                // jsonObj.push(detail);
                addTestingExplainList(explain,function(ret){
                    if (ret.result == true) {
                        //重新展示
        ...
        //提交后台编辑数据
        function editTestingExplainList(jsonObj_explains){
            var param = {
            ...
            editTestingExplain('{{URL::asset('')}}', param, function (ret) {
                console.log("editTestingExplain ret is ： "+JSON.stringify(ret))
                if (ret.result == true) {
                    return ret.result;
                } else {
                    layer.msg(ret.msg, {icon: 2, time: 1000})
        ...
        function addTestingExplainList(explain,callBack){
            ...
            editTestingExplain('{{URL::asset('')}}', param, callBack)
        }
    </script>
@endsection