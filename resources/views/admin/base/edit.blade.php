        ...
        <div id="tab-system" class="HuiTab">
            ...
            <div class="tabCon">
                <form class="form form-horizontal" method="post" id="form-baseInfo-edit">
                    ...
                    <div class="row cl" id="container_wechat">
                        <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>微信：</label>
                        <div class="formControls col-xs-8 col-sm-9">
                            @if($data['wechat'])
                                <img id="wechatPrv" src="{{$data['wechat'] }}" width="150" />
                            @else
                                <img id="wechatPrv" src="{{ URL::asset('/img/upload.png') }}" width="150" />
                            @endif
                            <span class="c-red">*请上传100*100尺寸的图片</span>
                            <input type="hidden" class="input-text" id="wechat" name="wechat" value="{{ isset($data['wechat']) ? $data['wechat'] : '' }}"  />
                        </div>
                    </div>
                    <div class="row cl">
...
//                        console.log(JSON.stringify(info));
            ...
            var uploader_wechat = Qiniu.uploader({
                runtimes: 'html5,flash,html4',      // 上传模式，依次退化
                browse_button: 'wechatPrv',         // 上传选择的点选按钮，必需
                container: 'container_wechat',//上传按钮的上级元素ID
                // 在初始化时，uptoken，uptoken_url，uptoken_func三个参数中必须有一个被设置
                // 切如果提供了多个，其优先级为uptoken > uptoken_url > uptoken_func
                // 其中uptoken是直接提供上传凭证，uptoken_url是提供了获取上传凭证的地址，如果需要定制获取uptoken的过程则可以设置uptoken_func
                uptoken: "{{$upload_token}}", // uptoken是上传凭证，由其他程序生成
...
//                        console.log(JSON.stringify(info));
                        ...
                        //获取上传成功后的文件的Url
                        var sourceLink = domain + res.key;
                        $("#wechat").val(sourceLink);
                        $("#wechatPrv").attr('src', qiniuUrlTool(sourceLink, "head_icon"));
                        // console.log($("#pickfiles").attr('src'));
                    },
                    'Error': function (up, err, errTip) {
                        //上传出错时，处理相关的事情
