        ...
        <form class="form form-horizontal" method="post" id="form-comment-edit">
            ...
            <div class="row cl">
                <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                    @if(!$data['status'])
                        <button onClick="league_editDo('{{$data['id']}}')"  class="btn btn-success radius" type="button">标记已联系</button>
                    @endif
...
@section('script')
    <script type="text/javascript">
        ...
        /* 点击审核通过 */
        function league_editDo(id) {
            var param = {
                id: id,
                _token: "{{ csrf_token() }}"
            }
            stampLeague('{{URL::asset('')}}', param, function (ret) {
                if (ret.result == true) {
